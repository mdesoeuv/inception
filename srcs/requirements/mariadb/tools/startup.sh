...
then
	...
	mysql -u root -p${MYSQL_ROOT_PASSWORD} -e "CREATE USER 'evaluator' IDENTIFIED BY '$MYSQL_PASSWORD'"
	mysql -u root -p${MYSQL_ROOT_PASSWORD} -e "GRANT USAGE ON $MYSQL_DATABASE.* TO 'evaluator'@'%' IDENTIFIED BY '$MYSQL_PASSWORD' WITH GRANT OPTION"
fi
